...
<body>
  ...
  <main class="news_page">
    <div class="news_wrap">
      ...
      <div class="news_image"> <img src="../images/news_blog/biyousitu.jpg" alt=""></div>
    </div>
    <div class="news_wrapwrap">
      <div class="news_list">
        <?php
        $counts = $db->query('select count(*) as cnt from blog_cms');
        $count = $counts->fetch_assoc();
        $max_page = floor(($count['cnt'] + 1) / 12 + 1);

        $stmt = $db->prepare('select id, category, imgfile, date, title from blog_cms order by id desc limit ?, 10');
        if (!$stmt) {
          die($db->error);
        }
        ...
        $stmt->bind_result($id, $category, $imgfile, $date, $title);
        while ($stmt->fetch()) :
          ...
          ?>
            ...
            <div class="recent_post_box">
              ...
              <div class="recent_post_box_wrap">
                ...
                <div class="recent_post_blog_date">
                  <p><?php echo h($date); ?></p>
                </div>
                <div class="recent_post_blog_title">
